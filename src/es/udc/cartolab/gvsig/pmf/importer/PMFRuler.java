...
public class PMFRuler implements Ruler {
    ...
    private static final Logger logger = Logger.getLogger(PMFRuler.class);
    /*
     ...
     * Comprador
     */
    private final static Pattern comunidadPattern = Pattern.compile("^\\d{8}$",
	    Pattern.CASE_INSENSITIVE);

    private final static Pattern viviendaPattern = Pattern.compile(
	    "^(\\d{8})vi\\d{3}$", Pattern.CASE_INSENSITIVE);

    private final static Pattern educativoPattern = Pattern.compile(
	    "^(\\d{8})ce\\d{2}$", Pattern.CASE_INSENSITIVE);

    private final static Pattern reunionesPattern = Pattern.compile(
	    "^(\\d{8})cr\\d{2}$", Pattern.CASE_INSENSITIVE);

    private final static Pattern saludPattern = Pattern.compile(
	    "^(\\d{8})cs\\d{2}$", Pattern.CASE_INSENSITIVE);

    private final static Pattern fuentePattern = Pattern.compile(
